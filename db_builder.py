...
from typing import Callable, Optional, Union, Generator, Dict, Any, Tuple
import schemas
import db
import os
import uuid
...
BATCH_SIZE = int(os.getenv("BATCH_SIZE", "100"))  # Number of files to process before committing
MAX_WORKERS = int(os.getenv("MAX_WORKERS", "4"))  # Number of parallel workers for metadata extraction
...
def main():
    ...
    try:
        ...
        log_and_print('OK', f"Cached {len(artist_cache)} artists and {len(album_cache)} albums")
        ...
        if last_processed:
            ...
            for file_path in file_generator:
                ...
                if str(file_path.absolute()) == last_processed:
                    log_and_print('OK', f"Skipped {skip_count} previously processed files")
                    break
        ...
        with tqdm(total=estimated_total, desc="Processing files") as pbar:
            for file_path in file_generator:
                ...
                if len(batch) >= BATCH_SIZE:
                    ...
                    log_and_print('INFO', f"Processed {processed_count} files ({files_per_second:.2f} files/sec), {success_rate:.1f}% success")
        ...
        log_and_print('OK', f"Database build complete!")
...
if __name__ == "__main__":
    if not MEDIA_FOLDER:
        log_and_print('ERROR', "Please set MEDIA_FOLDER environment variable either in the .env file or as a system variable.")
        exit(1)
    else:
